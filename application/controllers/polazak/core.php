...
class Core extends navigator 
{
    ...
    function view($from=0){
        ...
        $config['base_url'] = 'http://localhost/asp-timetable/polazak/core/view';
    ...
    function ajax_page_view($from = 0){
        ...
        $config['base_url'] = 'http://localhost/asp-timetable/polazak/core/ajax_page_view';
        $config['total_rows'] = $count;
        $config['per_page'] = $limit;
        $config['uri_segment'] = 4;
        $config['num_links'] = $total_page;
        ...
        $html = $this->load->view('polazak/search', $this->data,TRUE);
